...
test('2×2 左上角：左旋四次回到原锚点', () => {
  const tiles = [{ x: 1, y: 2, z: 0, suit: 'a' }];
  let t = tiles;
  for (let i = 0; i < 4; i += 1) {
...
test('rotate_left = 视觉逆时针 90°（屏幕坐标 x↓ y→）', () => {
  const tiles = [
    { x: 0, y: 0, z: 0, suit: 'A' },
    { x: 0, y: 2, z: 0, suit: 'B' },
    { x: 2, y: 0, z: 0, suit: 'C' },
  ...
  const at = (x, y) => r.find((t) => t.x === x && t.y === y)?.suit;
...
test('真实关卡左转后可重新编码（操作后按 z,x,y 规整）', () => {
  const s =
